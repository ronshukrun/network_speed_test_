...
def udp_offer_broadcast():
    offer_message = struct.pack('!IBHH', MAGIC_COOKIE, OFFER_TYPE, SERVER_UDP_PORT,SERVER_TCP_PORT)  # The UDP message is packaged using struct.pack. It packages data into binary format.
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_sock:
            ...
            while True:
                udp_sock.sendto(offer_message, ('<broadcast>',SERVER_UDP_PORT))  # The socket sends the offer message (offer_message) to anyone listening on the UDP port.
...
def handle_udp_request(data, client_address, udp_sock):
    try:
        ...
        magic_cookie, msg_type, file_size = struct.unpack('!IBQ', data)  # Information return back to parts Magic cookie, Message type and File size.
...
def handle_tcp_client(client_socket):
    ...
    """
    try:
        data = client_socket.recv(BUFFER_SIZE).decode().strip()  # Receive data from the client, decode it to a string, and strip any whitespace or newline characters.
        if not data.isdigit():
